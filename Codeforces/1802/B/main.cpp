...
void solve() {
    ...
    // before doctor comes, all new pigs go into different cages
    //  after doctor comes, if pig counts are even, we require n/2+1 cages
    //      otherwise we require n/2+1 cages

    int pc=0, avc=0, res=0;
    each(v, b) {
        if (v == 1) ++pc, ++avc;
        if (v == 2) avc = (pc > 0) ? pc/2 + 1 : 0;
        cmax(res, avc);
    }
