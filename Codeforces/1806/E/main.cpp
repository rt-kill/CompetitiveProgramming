...
int get_depth(int r) {
    if (depth[r] < 0) depth[r] = get_depth(p[r]) + 1;
    return depth[r];
}
...
ll pop_rwb(int r) {
    if (rwb[r] < 0) rwb[r] = pop_rwb(p[r]) + a[r]*a[botanc[get_depth(r)]];
    return rwb[r];
}
...
void solve() {
    ...
    p.rsz(n);
    p[0] = 0;
    For(i, 1, n) re(p[i]), --p[i];

    depth.assign(n, -1);
    depth[0] = 0;
    For(i, 0, n) get_depth(i);


    For(i, 0, n) {
        int d = get_depth(i);
        levels.rsz(max((int) levels.sz, d+1));
        levels[d].pb(i);
    }

    bot = levels.back().back();
    botd = get_depth(bot);

    botanc.assign(botd+1, 0);
    for(int tmp = bot; tmp != 0; tmp = p[tmp]) botanc[get_depth(tmp)] = tmp;
