...
void solve() {
    ...
    int zc = transform_reduce(all(a), 0, plus{}, [](auto v){ return v == 0; });
    int g1c = transform_reduce(all(a), 0, plus{}, [](auto v){ return v > 1; });

    if (zc <= (n+1)/2) { ps(0); return; }
    if (g1c > 0 || n == zc) { ps(1); return; }
    if (n-zc >= 1) { ps(2); return; }


    // unless over (n+1)/2 of a are 0, we can get mex of 0
    //  other
    //  otherwise, unless a is only 1's and 0's, (with exactly one 1), we have a mex of 2
    //  if there are multiple 1's, a
}
