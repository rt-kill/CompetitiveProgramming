...
void solve() {
    ...
    ll res = transform_reduce(all(a), 0ll, plus{}, [](auto v){ return abs(v); });
    if (n == 1) cmin(res, abs(a[0] - a[1]));
    if (n == 2) cmin(res, transform_reduce(all(a), 0ll, plus{}, [](auto v){ return abs(v-2); }));
    if (1^(n&1)) {
        ll cand = abs(*max_element(all(a)) - n);
        cand += transform_reduce(all(a), 0ll, plus{}, [](auto v){ return abs(v + 1); }) - abs(*max_element(all(a)) + 1);
        cmin(res, cand);
    }

    ps(res);

    // if a good array has a 0, all elements must be zero
    //  consider the subsequence of the n largest and n smallest values -- one of them must contain 0, the other is either all positive or negative

    // for n=1, good arrays are clear

    // suppose we have some good array q with a non-zero element
    //  consider for the k largest elements |∏qi| = ∏|qi|
    //  if they are all greater than 1, then this is greater than ∑|qi| > ∑|pi|

    // if at least half of the elements are greater than |1|, we have a problem

    // by inequality, ∏L + ∏R = C
    //  all positive elements in both must be 1
