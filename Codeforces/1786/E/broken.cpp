...
// For calculating the sum of hp's in a range
vll sumbit;
vll sumve;

// For calculating the number of mosters in a range
vll countsbit;
vll countsve;

// For calculating the sum of missing hp's in a range
vll mmbit;
vll mmve;

// For calculating the number sum of missing hp's in a range
vll mcbit;
vll mcve;

// For something?
vll mybit;
// a monotonic vector such that myve[i] >= i;
vll myve;
...
void init_bits() {/*{{{*/
    ...
    countsbit.assign(n+1, 0);
    countsve.assign(n+1, 0);

    mmbit.assign(n+1, 0);
    mmve.assign(n+1, 0);
    For(i, 1, n+1) {
        update_bit(mmbit, mmve, i, i);
    }

    mcbit.assign(n+1, 0);
    mcve.assign(n+1, 0);
    For(i, 1, n+1) {
        update_bit(mcbit, mcve, i, 1);
    }
...
void update_sum(ll hp) {/*{{{*/
    update_bit(sumbit, sumve, hp, sumve[hp] + hp);
}

ll query_sum(int l, int r) {
    ll sum = query_bit(&sumbit, l, r);
    return sum;
}/*}}}*/

void update_counts(ll hp) {/*{{{*/
    update_bit(countsbit, countsve, hp, sumve[hp] + 1);
}

ll query_counts(int l, int r) {
    ll sum = query_bit(&countsbit, l, r);
    return sum;
}/*}}}*/

void update_mm(ll hp) {/*{{{*/
    update_bit(mmbit, mmve, hp, 0);
}

ll query_mm(int l, int r) {
    ll sum = query_bit(&mmbit, l, r);
    return sum;
}/*}}}*/

void update_mc(ll hp) {/*{{{*/
    update_bit(mcbit, mcve, hp, 0);
}

ll query_mc(int l, int r) {
    ll sum = query_bit(&mcbit, l, r);
    return sum;
}/*}}}*/
...
void update_bits(int hp) {/*{{{*/
    // Do this first because it uses countsve
    update_rurq(hp);
