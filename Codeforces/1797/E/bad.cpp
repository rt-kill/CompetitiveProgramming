...
pi seg[4*N];    // seg[i].fi = end value for op2, seg[i].se = solution to q2
int lazy[4*N];
...
void prop_lazy(int i, int l, int r) {
    ...
    lazy[2*i] += clv, lazy[2*i+1] += clv;
    For(i, 0, clv) {
        // if cv == 1 we decrement cc by the number of nodes that are not 1 in the range
        // otherwise if no nodes in the range [l, r] have value cv, we decrement cc by cr
        // otherwise cv become phi[cv] and nothing else changes
    }
}
...
void update_node(int i, int l, int r) {/*{{{*/
    int m = (l+r)/2;
    auto [ lv, lc ] = seg[2*i];
    auto [ rv, rc ] = seg[2*i+1];

    int ld=0, rd=0;
    while(lv != rv) {
        if (lv > rv) lv = phi[lv], ++ld;
        if (lv < rv) rv = phi[rv], ++rd;
    }

    int nv = lv;
    int nc = (lc + ld*(m-l+1)) + (rc + rd*(r-(m+1)+1));
    seg[i] = { nv, nc };
}/*}}}*/
...
pi query_seg(int ql, int qr, int i=1, int l=0, int r=n-1) {/*{{{*/
    ...
    auto [ lv, lc ] = query_seg(ql, qr, 2*i, l, m);
    int lr = (m-max(ql,l)) + 1;
    auto [ rv, rc ] = query_seg(ql, qr, 2*i+1, m+1, r);
    int rr = (min(qr,r)-(m+1)) + 1;
...
void solve() {
    ...
    for(auto [t, l, r] : quer) {
        if (t == 1) update_seg(l, r);
        else ps(query_seg(l, r).se);
    }
