...
arr<lll, 2> dfs(int r) { // returns { grundy, max_possible_grundy }
    ...
    lll mx_chld_xor = 0;
    for(int i=63; i>=0; --i) {
        if(feq[i] == 1) mx_chld_xor += (1ll<<i);
        if(feq[i] >= 2) {
            mx_chld_xor += ((1ll<<(i+1))-1);
            break;
        }
    }

    lll mx_possible_grundy = accumulate(all(chld_mx_grundys), 0ll) + 1 + mx_chld_xor;
    // ll mx_possible_grundy = *max_element(all(chld_mx_grundys)) + 1 + mx_chld_xor;
    // ll mx_possible_grundy = 1 + mx_chld_xor;

    lll cur_grundy;
    if(c[r]) cur_grundy = accumulate(all(chld_grundys), 0ll) + 1 + mx_chld_xor;
    // if(c[r]) cur_grundy = *max_element(all(chld_grundys)) + 1 + mx_chld_xor;
    // if(c[r]) cur_grundy = 1 + mx_chld_xor;
