...
// vvll memo;
ve<unordered_map<int, ll>> memo;

vll inv;

ll dp(int loc, int cards) {
    if(loc == 0 && cards == 1) return 1;
    if(memo[loc].count(cards)) return memo[loc][cards];
    ...
    for(int roll=0; roll<min(cards, 6); ++roll) {
        ...
        res += dp(loc - num_cards_bef_loc_removed, cards - num_cards_rmved) * inv[min(cards, 6)];
    }
...
void solve() {
    ...
    inv.resize(100);
    for(int i=1; i<10; ++i) inv[i] = moddiv(1, i);

    // memo = vvll(n+10, vll(n+10, -1));
    memo = ve<unordered_map<int, ll>>(n+10);
