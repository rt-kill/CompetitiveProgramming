...
TrieNode *initTrie() {
    ...
    TrieNode* (&children)[26] = res->children;
...
void AddWord(TrieNode *root, string w) {
    ...
    each(a, w) {
        TrieNode* &next = (current->children)[a - 'a'];
        if (next == NULL) next = initTrie();
...
int SearchWord(TrieNode *root, string w) {
    TrieNode *current = root;
