...
void suffix_array() {
    ...
    for(int j=2; j<n; j<<=1) {
        ...
        For(i, 0, n) {
            int ind = parts[i][2] + j;
            parts[i][1] = (ind < n) ? parts[inds[ind]][0] : -1;
        }
...
void long_comm_pre() {
    ...
    int k=0;
    For(i, 0, n) {
        // If suffix i is the largest suffix
        if (ISA[i] == n-1) {
            k=0;
            continue;
        }

        // position of suffix next in order
        int j = SA[ISA[i] + 1];
        ...
        // lcp of posoition i and i+1 in sorted order
        lcp[ISA[i]] = k;
        k -= (k>0);
    }
}
