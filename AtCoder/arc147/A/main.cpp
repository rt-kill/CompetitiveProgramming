...
void solve() {
    ...
    // for complexity, lets first consider just the case with 2 elements
    //  after two operations, the first element becomes M%m, and the second element becomes m%(M%m)
    //      if M%m >= m/2, m%(M%m) <= m/2 in particular, m%(M%m) = m-(M%m)
    //      otherwise, M%m < m/2 and so m%(M%m) < m/2
    //      either way, since M > m, (M+m) is decreasing by over a factor of 2
    //      thus log2

    // now 3: x1->x1%x3, x2->x2%(x1%x3), x3->x3%(x2%(x1%x3))
    //          similarly, x2'+x3' <= x3
    //          additionally, x1'+x2' <= x2
    //          and x1' + x3' <= x3
    //      so x1'+x2'+x3' <= x2/2+x3 <= (x1+x2+x3)/2

    // the key detail is that whenever a number is modded by a number less that it, it decreases by at least a factor of 2
    //  so after 1 full round, all numbers will have decreased by at least 2


    sort(all(a));
    deque<int> deck(all(a));
    ...
    while(deck.sz > 1) {
        ...
        int m = deck.front();
        int M = deck.back(); deck.pop_back();

        int nv = M%m;
        if (nv > 0) deck.push_front(nv);
    }
