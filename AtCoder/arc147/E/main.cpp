...
void solve() {
    ...
    a.rsz(n), b.rsz(n);
    For(i, 0, n) tie(a[i], b[i]) = pairs[i];

    sort(all(pairs), [](auto l, auto r){ return l.se < r.se || (l.se == r.se && l.fi < r.fi); });
    sort(all(a)), sort(all(b));

    inds.rsz(n);
    Rof(i, 0, n) {
        int j = lb(all(a), b[i]) - a.begin();
        inds[i] = j;
        if (inds[i] > i) { ps(-1); return; }
    }
    ...
    int res = 0;
    Rof(i, 0, n) {
        ++fvs[inds[i]]; // i must be matched with something <= inds[i]
        // question: is a[i] located above inds[i], and if so, is there a free slot?
    }



    // a solution exists only if there is a perfect matching
    //  there is a perfect mathching iff for all i, inds[i] <= i
    // suppose for some S ⊂ N, we dont swap the scores of S
    //  a perfect mathcing exists iff for all i ∉ S, inds[i] - C[inds[i]] <= i - C[i]

    // if for some i, index(a[i]) <= inds[i], i may point to itself
}
