...
void solve() {
    ...
    // if there are less than L/2 people currently, the next group can always find a seat
    //  if there is a group of 2 after the first L/2 people have taken a seat,

    int li=0, ri=l-1;
    int res=1;
    For(i, 0, n) {
        if (a[i] == 2 && ri <= li) res=0;
        if (a[i] == 1) li += 2;
        if (a[i] == 2) ri -= 3;
    }
