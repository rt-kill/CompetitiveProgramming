...
void solve() {
    ...
    sb.rsz(n);
    For(i, 0, n) sb[a[i]-1] = b[i];

    map<int, int> mymap;
    int lis = 0;
    Rof(i, 0, n) {
        auto it = mymap.lower_bound(sb[i]);
        int nv= (it != mymap.end()) ? 1 + (*it).se : 1;
        if (it != mymap.begin() && (*--it).se == nv) mymap.erase(it);
        mymap[sb[i]] = nv;
        cmax(lis, nv);
    }
    ...
    // in reverse order
    //  if a member larger than b[i] has been seen,
}
