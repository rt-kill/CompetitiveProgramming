...
void solve() {
    ...
    int off = *min_element(all(a));
    each(v, a) v -= off;

    int gcd=0;
    each(v, a) {
        int m1 = max(v, gcd), m2 = min(v, gcd);
        while(m2 > 0) m1 %= m2, swap(m1, m2);
        gcd = m1;
    }
    ...
    // if we choose M=2, we get 2 elements
    // if gcd(all(a)) > 1, we choose M = gcd(all(a))

    // if we want to get 1 element, it would mean for some M
    //      a[0]%M = a[i]%M
    // subtract the smallest element from everything and then find the gcd
}
