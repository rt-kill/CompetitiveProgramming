...
int myRec(int l, int r, char lc, char rc) {
    if (l == r) return 1;
    if (r - l + 1 == 2) return lc == rc;
    if (lc != rc) return (lc == 'B' && rc == 'A');

    int res = myRec(l+1, r-1, s[l+1], s[r-1]);
    if (lc == 'A' && s[l+1] != 'B') res |= myRec(l+1, r-1, 'B', s[r-1]);
    if (rc == 'B' && s[r-1] != 'A') res |= myRec(l+1, r-1, s[l+1], 'A');
    return res;
}

void solve() {
    re(n, s);
    int res = myRec(0, n-1, s[0], s[n-1]);
    ps(res ? "Yes" : "No");
}
