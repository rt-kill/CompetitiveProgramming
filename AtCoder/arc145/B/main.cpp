...
void solve() {
    ...
    if (a <= b) { ps(max(0ll, n-(a-1))); return; }

    n = max(0ll, n-(a-1));

    ll fullBlocks = n/a;
    ll gamesPerFullBlock = b;

    ll remBlock = n%a;
    ll remBlockWins = min(remBlock, b);

    ll res = fullBlocks*gamesPerFullBlock + remBlockWins;
    ps(res);

    // alice will lose all games with n < a
    //  in the interval [a, 2a), alice will win games [a, a+(b-1)], and lose the rest

    // if a <= b, alice will win all games with [n >= a], and lose all others
    // if a > b, alice will win all games with [ n%a < b ] and lose all others
}
