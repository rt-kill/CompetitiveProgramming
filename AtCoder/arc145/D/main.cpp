...
void solve() {
    ...
    // 0, 1, _, 3, 4, _, _, _, _, 9, 10, _, 12, 13,

    // lets just try to find a set S of size N without the sum condition
    //  S := { 2^k } is a valid set, but its size is log(10^7) which is very small

    // start with S = { 0, 1 }
    //  for (k, 2, Max), if for all (i, j) in S, (2*S[i] - S[j] != k), add K to S

    // is there a set B such that for all x, y \in B, 2*x-y \notin B?

    // suppose that x and y are both prime,
    //      2*x - y = p
    //      (5, 3, 7)
    //      (13, 7, 19)
}
