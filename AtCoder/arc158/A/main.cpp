...
void solve() {
    ...
    re(x[0], x[1], x[2]);
    sort(all(x));
    x[2] -= x[0], x[1] -= x[0], x[0] -= x[0];

    if (((x[0]&1)|(x[1]&1)|(x[2]&1)) != ((x[0]&1)&(x[1]&1)&(x[2]&1))) { pln(-1); return; }
    if ((x[0] + x[1] + x[2])%6 != 0) { pln(-1); return; }

    int res = 0;

    int m1 = x[1]/2, m2 = (x[2]-x[1])/2;
    int mm = min(m1, m2);
    res += mm;

    x[0] += 4*mm, x[1] += 2*mm;
    if (x[0] == x[1]) { res += (x[2]-x[1])/3; }
    if (x[1] == x[2]) { res += (x[1] - x[0])/3; }
    pln(res);
}
